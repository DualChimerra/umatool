...
The interface is in Russian. Names and in-game terms are deliberately not translated —
skill, uma, card and racecourse names, `Turf` / `Dirt`, `Sprint` / `Mile` / `Medium` /
`Long`, the four running styles, `SSR` / `SR` / `R` and the five stats all stay exactly as
the Global client spells them, because that is what a player reads on their own screen.
Everything around them — headings, explanations, advice and the rendered activation
conditions — is Russian. The condition sentences are generated by
`scripts/lib/conditions.mjs`, so a data rebuild keeps them Russian.
...
two. Cards outside the collection therefore stay visible in the picker, badged *друг*, and
are ranked by what they would add like everything else; once the borrow is spent they
remain listed but disabled, so it is clear what taking the friend's card cost. A
**Все / Мои / Карты друга** switch narrows the list either way.
